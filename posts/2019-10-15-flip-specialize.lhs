---
title: "Specialization using flip"
description: A simple Haskell trick I discovered recently
tags: haskell
---
...
alert_ -- they turn out not to be necessary in the end:
...
In Haskell, it is idiomatic design pattern to specify _arguments that are
unlikely to change in between function calls_ first.
...
counting items, we can _specialize_ this function by _partially applying it_ to
obtain a function which adds a count:

> increaseCount :: Ord k => k -> Int -> M.Map k Int -> M.Map k Int
...
System.Control.Concurrent.writeChan  :: Chan a -> a -> IO ()
System.Control.Concurrent.modifyMVar :: MVar a -> (a -> IO (a, b)) -> IO b
~~~~~
...
our database the following top-level function:
...
If we want a specialized version of this, we can do something like, which
requires us to bind `h`, which sometimes feels a bit awkward:
...
... probably not?  While it is an interesting trick, unless it becomes a real
pain point for you, I see little wrong with just writing:
