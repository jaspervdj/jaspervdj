---
title: "On Lazy I/O and graphs: Winterfell to King's Landing"
...
We model the different cities in a straightforward way.  Except for a unique ID
used to identify them, they also have a name, a position (_X,Y_ coordinates) and
a list of reachable cities, with an associated time (in days) it takes to travel
there.  This travel time, also referred to as the _cost_, is not necessarily
deducable from the sets of _X,Y_ coordinates: some roads are faster than others.
...
`CityId`s both advantages and disadvantages.
...
The costs have been generated by multiplying the actual distances with a random
number uniformly chosen between `0.6` and `1.4`.
...
[Dijkstra's graph search algorithm](dijkstra) that is able to work with infinite
...
Firstly, we have a priority queue of nodes we will visit next in `ssQueue`,
including back references.  Secondly, we have the `ssBackRefs` map.  That one
serves two purposes: to keep track of which nodes we have already explored (the
keys in the map), and to keep the back references of those locations (the values
in the map).
