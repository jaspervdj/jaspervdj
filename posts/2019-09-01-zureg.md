...
blogpost explains why we decided to go this route, and we go into its design
just a little bit.
...
First, I'd like to go into how the project is build and compiled.  In a
nutshell: it's not great, but I do think it makes a good cookbook of how to do
things by hand.
...
so it is all done from scratch -- and it's surprisingly short.  However, if I
were to start a new project, I would probably use one of these frameworks:
...
[^1]: In serverless terminology, it seems to common to refer to lambdas that
    ...
    the code works and is reusable rather than how it is deployed -- but coming
    from a functional programming perspective I do like the sound of
    "fat lambda" a lot.
...
[scanner.js](https://github.com/zfoh/zureg/blob/master/static/scanner.js) is a
